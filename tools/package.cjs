...
function generatePackage() {
	...
	const rootPackage = JSON.parse(fs.readFileSync(path.join(getArgs().root, 'package.json'), 'utf8'));
	const packageJson = packageMetadata(rootPackage.version ?? '0.0.0', rootPackage.dependencies);
